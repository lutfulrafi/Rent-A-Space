...
function userEmailExists($email){
    ...
    if(!mysqli_stmt_prepare($stmt,$sql)){
        header("location:hostregister.php?error=stmtfailed");
        exit();
    }
...
function createUser( $username, $email, $password){
    include("db_connect.php");
    $sql="INSERT INTO `host` (Name, email, password) VALUES(?, ?, ?) ";
    $stmt=mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt,$sql)){
        header("location:hostregister.php?error=stmtfailed");
        exit();
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("location:hostLogin.php?error=none");
    exit();
...
function logIN_password_match($password,$useremail):bool{
    ...
    if($userEmailExists=== false){
        header("location:hostLogIn.php?error=wronglogin");
        exit();
    }
...
function loginAdmin($useremail, $password){
    ...
    if($checkpwd === false){
        ...
        header("location: ./dashboard/html/admin/adminDashboard.php");
        exit();
    }
}
?>