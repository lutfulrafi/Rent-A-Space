...
if (isset($_POST['submit'])) {
    ...
    require_once 'hostRegisterUtils.php';

    if (emptyInputSignup($username, $email, $password, $cpassword) !== false) {
        header("location:hostregister.php?error=emptyinput");
        exit();
    }
    if (userEmailExists($email) !== false) {
        header("location:hostregister.php?error=duplicateEmailorMobile");
        exit();
    }
    if (invalidEmail($email) !== false) {
        header("location:hostregister.php?error=invalidEmail");
        exit();
    }
    if (passwordMatch($password, $cpassword) !== false) {
        header("location:hostregister.php?error=passwordDontMatch");
        exit();
    }
...
<body>
    ...
    <div class="container">
        ...
        <form action="hostregister.php" method="post" id="form" class="form">
